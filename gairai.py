...
# 定数定義
SHIFT_TYPES = ['日', 'OP日', '内日', 'A', 'B', '当直', 'OFF', '遅', '半遅', '年休', '特休', '夏休', '/年', '年/', '/半', '半/', '年/半', '/特', '特/', '/夏', '夏/', '研修', '/研', '研/', '健/年', '健/半', '健/', '---', '介護休暇', '病欠', '産休', '育休', '学']
...
# ★Aは休日カウント対象から除外
HOLIDAY_WORK_SHIFTS = ['日', '遅', '半遅'] 
WORK_SHIFTS = ['日', 'OP日', '内日', 'A', 'B', '当直', '遅', '半遅', '/半', '半/', '/特', '特/', '/夏', '夏/', '研修', '/研', '研/', '学']
E_CAPABLE_SHIFTS = ['日', 'OP日', '内日', 'B', '半遅']
REST_AND_A_SHIFTS = ['OFF', '年休', '特休', '夏休', '/年', '年/', '/半', '半/', '年/半', '/特', '特/', '/夏', '夏/', '健/年', '健/半', '健/', 'A', '介護休暇', '病欠', '産休', '育休']
# ==========================================
# 1. データ生成・読み込み・保存関数
# ==========================================
...
def get_monthly_schedule(year, month):
    ...
    if key in data:
        try:
            gen_df = pd.DataFrame(data[key]['gen']).replace({None: np.nan})
            ...
            e_df = pd.DataFrame(data[key]['e_gen']).replace({None: np.nan})
            ...
            if s in p_gen.index and last_day in p_gen.columns:
                ...
                if val_last == 'A':
                    ...
                    if (val_last == '遅' and last_date_obj.weekday() == 5 and not jpholiday.is_holiday(last_date_obj)) or \
                       (val_last == '半遅' and (last_date_obj.weekday() == 6 or jpholiday.is_holiday(last_date_obj))):
                        day1 = f"{target_year}-{target_month:02d}-01"
                        if day1 in date_cols and col_df.at[s, day1] not in ['green', 'red']:
                            gen_df.at[s, day1] = 'OFF'
                            col_df.at[s, day1] = 'green'
        ...
        if c['year'] == year and c['month'] == month:
            ...
            if d_s in cols_local:
                for sid in c['members']:
                    if sid in gen_df.index:
                        if col_df.at[sid, d_s] != 'green':
                            wished_off = any(w['staff_id'] == sid and w['date'] == d_s and w['shift'] in ['OFF', '年休', '特休', '夏休', '/年', '年/', '/半', '半/', '年/半', '/特', '特/', '/夏', '夏/', '介護休暇', '病欠', '産休', '育休'] for w in wishes_local)
...
def create_default_staff_data():
    return pd.DataFrame(columns=["職員番号", "名前", "所属", "役職", "パスワード", "表示順", "夜勤上限", "休日日勤上限", "遅出半遅上限", "待機上限", "DM", "リブレ", "学生"])
...
def load_wishes_data():
    if os.path.exists(WISHES_FILE):
        try: 
            return pd.read_csv(WISHES_FILE).to_dict('records')
        except pd.errors.EmptyDataError: 
            return []
    return []
...
def get_allowed_shifts(role):
    ...
    if role == "事務": return ['OFF', '年休', '特休', '夏休', '/年', '年/', '/半', '半/', '年/半', '/特', '特/', '/夏', '夏/', '研修', '/研', '研/']
    elif role == "CW": return ['日', 'OP日', '内日', '遅', 'OFF', '年休', '特休', '夏休', '/年', '年/', '/半', '半/', '年/半', '/特', '特/', '/夏', '夏/', '研修', '/研', '研/']
    elif role == "師長": return ['日', 'OP日', '内日', '当直', 'OFF', '年休', '特休', '夏休', '/年', '年/', '/半', '半/', '年/半', '/特', '特/', '/夏', '夏/', '研修', '/研', '研/']
    elif role == "CE": return ['日', 'OP日', '内日', '遅', '半遅', 'OFF', '年休', '特休', '夏休', '/年', '年/', '/半', '半/', '年/半', '/特', '特/', '/夏', '夏/', '研修', '/研', '研/']
    elif role == "看護師": return ['日', 'OP日', '内日', 'A', 'B', '遅', '半遅', 'OFF', '年休', '特休', '夏休', '/年', '年/', '/半', '半/', '年/半', '/特', '特/', '/夏', '夏/', '研修', '/研', '研/']
...
def check_shift_rule(role, date_obj, shift):
    ...
    if role == "事務" and is_hol and shift not in ['OFF', '年休', '特休', '夏休', '/年', '年/', '/半', '半/', '年/半', '/特', '特/', '/夏', '夏/']:
        return f"🚫 {d_str}: 事務職は土日祝に勤務希望を出せません。"
    if is_hol and shift in ['OP日', '内日']:
        return f"🚫 {d_str}: 土日祝日に「OP日」や「内日」は選択できません（「日」などを選択してください）。"
...
@st.dialog("⚠️ 変更の確認")
def confirm_sensitive_changes(changes, f_ids, new_df_subset, e_changed, new_e_data, year, month):
    ...
    with col1:
        if st.button("変更する (反映)", type="primary"):
            ...
            st.session_state['generated_df'].loc[f_ids] = new_df_subset
            for c in changes: 
                st.session_state['meta_color_df'].at[c['idx'], c['col']] = 'green'
                
            if e_changed:
                for col_date in new_e_data.columns:
                    if st.session_state['e_schedule_df'].at['E', col_date] != new_e_data.at['E', col_date]:
                        st.session_state['meta_e_color_df'].at['E', col_date] = 'green'
                        st.session_state['e_schedule_df'].at['E', col_date] = new_e_data.at['E', col_date]
...
def pinpoint_fill_dialog(date_strs_local, col_map_local, staff_df_local, wishes_local, year, month):
    ...
    hole_shift = st.selectbox("📝 補充するシフト", options=["日", "遅", "半遅", "A", "B", "OP日", "内日"])
    ...
    if st.button("🚀 補充を実行する", type="primary"):
        ...
        for s in current_sch.index:
            for d_str in date_strs_local:
                ...
                if pd.notna(val):
                    ...
                    if val == '半遅': half_late_counts[s] += 1
        ...
        for sid in staff_df_local["職員番号"].values:
            ...
            if pd.isna(current_val) or current_val == 'OFF' or current_val == '---':
                ...
                if any(w['shift'] in ['OFF', '年休', '特休', '夏休', '産休', '育休', '病欠', '介護休暇'] for w in wished):
                    continue
...
if app_mode == "👤 スタッフ入力画面":
    ...
    # ★ 安全装置：文字に揃えて検索し、もし見つからなかったら強制ログアウトしてエラーを防ぐ
...
# === スタッフマスタ管理 ===
    ...
    with tab_shift:
        ...
        with st.expander("⚙️ 外来 日勤必要人数設定 (Ns等)", expanded=False):
            st.caption("※日勤計は休日は「師長」も含めて計算されます。土曜は遅出1名、日曜・祝日は半遅1名を絶対条件とします。")
            default_counts = []
            for d in dates:
                ...
                default_counts.append({"日付": d, "曜日": day_label, "Ns必要数(AM)": req_am, "Ns必要数(PM)": req_pm, "遅出必要数": req_late, "半遅必要数": req_hl})
                
            req_df = pd.DataFrame(default_counts)
            edited_req_df = st.data_editor(req_df[["曜日", "Ns必要数(AM)", "Ns必要数(PM)", "遅出必要数", "半遅必要数"]], use_container_width=True, disabled=["曜日"], hide_index=True)
            req_dict = {req_df.iloc[i]["日付"].strftime('%Y-%m-%d'): {"am": row["Ns必要数(AM)"], "pm": row["Ns必要数(PM)"], "late": row["遅出必要数"], "half_late": row["半遅必要数"]} for i, row in edited_req_df.iterrows()}
        ...
        if st.session_state.get('run_generation'):
            ...
            with st.spinner("処理中..."):
                            ...
                            for sid in c['members']:
                                if sid in schedule.index:
                                    if meta_color.at[sid, d_s] != 'green':
                                        wished_off = any(w['staff_id'] == sid and w['date'] == d_s and w['shift'] in ['OFF', '年休', '特休', '夏休', '/年', '年/', '/半', '半/', '年/半', '/特', '特/', '/夏', '夏/', '介護休暇', '病欠', '産休', '育休'] for w in current_wishes)
                                        if not wished_off:
                ...
                for w in blue_w:
                    ...
                    if role == '事務' and is_holiday_jp(cur_obj) and shift not in ['OFF', '年休', '特休', '夏休', '/年', '年/', '/半', '半/', '年/半', '/特', '特/', '/夏', '夏/', '介護休暇', '病欠', '産休', '育休']: valid = False
                    ...
                    if valid:
                        ...
                        if shift == '半遅': half_late_counts[sid] += 1
                ...
                def get_total_fixed_offs(target_sid):
                    ...
                    for wd in dates:
                        ...
                        if pd.notna(val):
                            ...
                            elif val in ['/半', '半/', '半遅', '年/半', '健/半', '/夏', '夏/']: cnt += 0.5
                        else:
                            ...
                            if wished:
                                ...
                                elif w_val in ['/半', '半/', '半遅', '年/半', '健/半', '/夏', '夏/']: cnt += 0.5
                    return cnt
                ...
                for i, d in enumerate(dates):
                    ...
                    if is_holiday_jp(d):
                        current_hl = (schedule.loc[tgt_ids_holiday, d_s] == '半遅').sum() 
                    else:
                        current_hl = (schedule.loc[all_ns_ids, d_s] == '半遅').sum() 
                ...
                pm_shifts = ['日', '遅', '半遅', '年/', '半/', '特/', '年/半', '健/', '夏/', '研/'] 
                ...
                for i, d in enumerate(dates):
                    ...
                    if is_hol:
                        current_shifts = schedule.loc[tgt_ids_holiday, d_s] 
                        cands_pool_day = tgt_ids_holiday
                    else:
                        current_shifts = schedule.loc[all_ns_ids, d_s] 
                        cands_pool_day = tgt_ids_weekday
                        
                    cur_am = current_shifts.isin(am_shifts).sum()
                    cur_pm = current_shifts.isin(pm_shifts).sum()
                    ...
                    if needed_day > 0:
                        ...
                        for sid in cands_to_use:
                            ...
                            # ★休日の日勤にはオペ室スタッフが2人以上にならないようにする（最大1人まで）
                ...
                for sid in sids:
                    ...
                    offs_05 = schedule.loc[sid].isin(['/半', '半/', '半遅', '年/半', '健/半', '/夏', '夏/']).sum()
                ...
                for i, d_s in enumerate(cols):
                    if meta_e_color.at['E', d_s] != 'green':
                        primary_cands = []   # 第一候補（日勤系）
                        secondary_cands = [] # 第二候補（どうしても！の時の B・半遅）
                        ...
                        for s in sids:
                            ...
                            if e_limit > 0 and e_counts[s] < e_limit:
                                ...
                                # ① 第一候補（日・OP日・内日 で、前日当直でない人）
                                if current_shift in ['日', 'OP日', '内日']:
                                    if not is_post_tochoku:
                                        primary_cands.append(s)
                                # ② 第二候補（B、半遅）
                                elif current_shift in ['B', '半遅']:
                                    secondary_cands.append(s)
                                # ※「遅」は候補から完全に除外
                        
                        # 候補の決定（第一候補がいればそれを使う、いなければ第二候補）
                        cands_to_use = primary_cands if primary_cands else secondary_cands
        ...
        if st.session_state['generated_df'] is not None:
            ...
            for sid in f_ids:
                ...
                hl_cnt = int((row == '半遅').sum())
                off_cnt = float((row == 'OFF').sum() + row.isin(['/半', '半/', '半遅', '年/半', '健/半', '/夏', '夏/']).sum() * 0.5)
                paid_cnt = float((row == '年休').sum() + row.isin(['/年', '年/', '健/年', '年/半']).sum() * 0.5)
                e_cnt = 0
                ...
                agg_data.append({'職員番号': sid, '夜勤': str(night_cnt), '待機': str(e_cnt), '遅出': str(late_cnt), '半遅': str(hl_cnt), 'OFF': off_str, '年休': paid_str})
            ...
            for n in ["AM", "PM", "待機"]:
                ...
                for ds in date_strs:
                    if n == "待機":
                        ...
                        data = full_sch.loc[[s for s in pool if s in full_sch.index], ds]
                        r[col_map[ds]] = str(data.isin(am_shifts if n=="AM" else pm_shifts).sum())
                rows.append(r)

            rows.append({"名前": "━━━━━━━━━━━━━━━━", "所属": "区切り", "役職": ""})
            ...
            e_staff_names = ["", "未定"] + staff_df[staff_df['待機上限'] > 0]['名前'].tolist()
            special_leaves = ['介護休暇', '病欠', '産休', '育休']
            ...
            with c_save1:
                if st.button("🔄 編集内容を確定して一括保存する", type="primary", use_container_width=True):
                    ...
                    current_e_base = st.session_state['e_schedule_df'].copy()
                    current_shift_base = st.session_state['generated_df'].loc[f_ids].copy()
                    ...
                    for sd, ds in rev_col_map.items():
                        ...
                        old_val = clean_val(current_e_base.at['E', ds])
                        
                        if new_val != old_val:
                            e_changed = True
                            st.session_state['e_schedule_df'].at['E', ds] = new_val if new_val != "" else np.nan
                            st.session_state['meta_e_color_df'].at['E', ds] = 'green'
                    ...
                    for name_idx, row in edited_all.iterrows():
                        ...
                        if sid not in current_shift_base.index: continue
                        
                        for sd, ds in rev_col_map.items():
                            new_val = clean_val(row[sd])
                            old_val = clean_val(current_shift_base.at[sid, ds])
                            
                            if new_val != old_val:
                                shifts_changed = True
                                st.session_state['generated_df'].at[sid, ds] = new_val if new_val != "" else np.nan
                                st.session_state['meta_color_df'].at[sid, ds] = 'green'
                                ...
                                if new_val == "A":
                                    ...
                                    if idx + 1 < len(date_strs):
                                        ...
                                        st.session_state['generated_df'].at[sid, next_ds1] = "B"
                                        st.session_state['meta_color_df'].at[sid, next_ds1] = 'green'
                                    if idx + 2 < len(date_strs):
                                        next_ds2 = date_strs[idx + 2]
                                        st.session_state['generated_df'].at[sid, next_ds2] = "OFF"
                                        st.session_state['meta_color_df'].at[sid, next_ds2] = 'green'

                                # 🌟 日曜日の「半遅」 -> 翌日OFF の自動連動ロジック 🌟
                                if new_val == "半遅":
                                    d_obj = datetime.datetime.strptime(ds, '%Y-%m-%d').date()
                                    # weekday()の「6」は日曜日を意味します
                                    if d_obj.weekday() == 6:
                                        idx = date_strs.index(ds)
                                        # 翌日(月曜日)が存在すればOFFを入れる
                                        if idx + 1 < len(date_strs):
                                            next_ds1 = date_strs[idx + 1]
                                            st.session_state['generated_df'].at[sid, next_ds1] = "OFF"
                                            st.session_state['meta_color_df'].at[sid, next_ds1] = 'green'
                    ...
                    if d_s in date_strs:
                        for sid in c['members']:
                            if sid in fin_sch.index:
                                ...
                                if actual_shift in ['OFF', '年休', '特休', '夏休', '/年', '年/', '/半', '半/', '年/半', '/特', '特/', '/夏', '夏/', '健/半', '健/年', '健/', '介護休暇', '病欠', '産休', '育休'] and not wished_off:
                                    errs.append(f"⚠️ {col_map[d_s]}: {staff_name}さんは「{c['committee']}」の予定ですが、お休み系または健診シフト({actual_shift})になっています")
                                elif actual_shift in ['遅', '半遅', 'A', 'B', '当直']:
            ...
            for sid in f_ids:
                if sid in fin_sch.index:
                    ...
                    offs_05 = fin_sch.loc[sid].isin(['/半', '半/', '半遅', '年/半', '健/半', '/夏', '夏/']).sum()
            ...
            for d_s in date_strs:
                ...
                if is_holiday_jp(d_obj):
                    col_data = fin_sch.loc[count_all_ids, d_s] 
                else:
                    col_data = fin_sch.loc[count_ns_ids, d_s] 
                ...
                if is_holiday_jp(d_obj):
                    for s in fin_sch.index:
                        ...
                        if val in ['OP日', '内日']:
                            ...
                            errs.append(f"❌ {col_map[d_s]}: 土日祝ですが、{staff_name}さんに「{val}」が入力されています")

                real_am = col_data.isin(am_shifts).sum()
                real_pm = col_data.isin(pm_shifts).sum()
                real_late = (col_data == '遅').sum()
                real_hl = (col_data == '半遅').sum()
                req_am = req_dict[d_s]["am"]
                req_pm = req_dict[d_s]["pm"]
                req_late = req_dict[d_s]["late"]
                req_hl = req_dict[d_s]["half_late"]
                ...
                if is_sun_or_hol:
                    ...
                    if real_hl != req_hl: errs.append(f"❌ {col_map[d_s]}: 日曜・祝日の半遅人数が絶対条件({req_hl}人)と異なります（現在 {real_hl}人）")
                    ...
                    if real_hl < req_hl: errs.append(f"💡 {col_map[d_s]}: 半遅不足 ({real_hl}/{req_hl})")

                # ★ 休日の日勤におけるオペ室スタッフの人数をチェック
                if is_holiday_jp(d_obj):
                    op_day_count = sum(1 for s in col_data.index if fin_sch.at[s, d_s] in am_shifts and staff_df[staff_df['職員番号']==s]['所属'].iloc[0] == 'オペ室')
                    if op_day_count >= 2:
                ...
                if is_sun_or_hol:
                    if d_s in date_strs[:-1]: 
                        next_d_s = date_strs[date_strs.index(d_s) + 1]
                        for s in fin_sch.index: 
                            if fin_sch.at[s, d_s] == '半遅':
                                n_val = fin_sch.at[s, next_d_s]
                                if pd.notna(n_val) and n_val not in REST_AND_A_SHIFTS and n_val != "" and n_val != "---":
                                    staff_name = staff_df[staff_df['職員番号']==s]['名前'].iloc[0]
                                    errs.append(f"⚠️ {col_map[d_s]}: {staff_name}さんの半遅出の翌日が「{n_val}」になっています (お休み系かAであるべきです)")
                ...
                if pd.isna(e_name) or str(e_name).strip() == "" or e_name == "未定":
                    ...
                    if len(e_sids) > 0:
                        ...
                        if staff_df[staff_df['職員番号'] == sid]['待機上限'].values[0] == 0: errs.append(f"❌ {col_map[d_s]}: 待機上限0の {e_name} さんが待機に割り当てられています")
            
            def get_err_priority(msg):
                if msg.startswith("❌"):
                    if "【公休数】" in msg:
                        return 2  # ❌グループの中で後回し（下に表示）
                    return 1      # 重大な❌（Aがいない等）を一番上に！
            ...
            # (1) 従来のCSVダウンロード
            out_csv = all_view.reset_index().drop(columns=['夜勤', '待機', '遅出', '半遅', 'OFF', '年休', '所属', '役職'], errors='ignore')
            ...
            with c_dl2:
                ...
                # 行1: タイトルと注意書き（後で各セルに個別に入力するため空行）
                ex_data.append([""] * (4 + len(dates) + 6))
                ...
                weekday_row = ["所属", "役職", "氏名／曜日", "勤務形態"] + [wd_names[d.weekday()] for d in dates] + ['夜勤', '待機', '遅出', '半遅', 'OFF', '年休']
                ...
                for label, s_list in summary_rows:
                    ...
                    for ds in date_strs:
                        ...
                        count = full_sch.loc[target_pool, ds].isin(s_list).sum()
                ...
                for g_idx, grp in enumerate(GROUPS):
                    ...
                    for sid in grp_staff['職員番号']:
                        ...
                        raw_name = grp_staff.set_index("職員番号").at[sid, "名前"]
                        role = grp_staff.set_index("職員番号").at[sid, "役職"]
                        ex_row = [grp, role, raw_name, "正職"] 
                        ...
                        for c_idx, ds in enumerate(date_strs):
                            ...
                            if c_ref == 'red': cell_styles[(curr_row_idx, c_idx + 4)] = "FF0000"
                            elif c_ref == 'blue': cell_styles[(curr_row_idx, c_idx + 4)] = "0000FF"
                                
                        for agg in ['夜勤', '待機', '遅出', '半遅', 'OFF', '年休']:
                ...
                with pd.ExcelWriter(output, engine='openpyxl') as writer:
                    ...
                    # 1行目: 令和タイトルとカラー注意書き
                    # XML破損エラーを回避するため、リッチテキストを使わずにセルを分割して横に並べます
                    target_off = agg_df['OFF'].iloc[0] if not agg_df.empty else "9"
                    
                    ws['C1'].value = f"{era_str}　"
                    ws['C1'].alignment = Alignment(horizontal='right', vertical='center')
                    ws['C1'].font = Font(bold=True)
                    
                    ws['D1'].value = "赤希望2回"
                    ws['D1'].font = Font(color="FF0000", bold=True)
                    ws['D1'].alignment = Alignment(horizontal='center', vertical='center')
                    
                    ws['E1'].value = "　青希望2回（変更しても可能）"
                    ws['E1'].font = Font(color="0000FF", bold=True)
                    ws['E1'].alignment = Alignment(horizontal='left', vertical='center')
                    
                    ws['I1'].value = f"　連続しての4日以上のOFFは師長へ報告必須　　OFF {target_off}回"
                    ...
                    for i, d in enumerate(dates):
                        ...
                        if color:
                            ws.cell(row=2, column=col_idx).font = Font(color=color, bold=True)
                            ws.cell(row=3, column=col_idx).font = Font(color=color, bold=True)

                    # スタッフの希望色（赤・青）を反映
                    for (r_i, c_i), hex_color in cell_styles.items():
                        ws.cell(row=r_i+1, column=c_i+1).font = Font(color=hex_color, bold=True)
